...
function computeUserScore(
	...
	const telegramScore = digitQuery
		...
		: 0
	const idScore = digitQuery
		? scoreNumericMatch(digitQuery, idDigits) * 0.75
		: 0
	...
	const similarityBonus =
		typeof similarityScore === 'number' ? Math.round(similarityScore * 35) : 0
...
export interface IStorage {
	...
	countRecentMessages(params: { userId: number; since: Date }): Promise<number>
	updateMessage(
		id: number,
...
export class DatabaseStorage implements IStorage {
	...
	async searchUsers(params: {
		...
		const baseQuery = db
			.select({ user: users, similarity: similarityScore })
			.from(users)
		...
		const scored = rows
			...
			.sort((a, b) => {
				...
				const aTime = a.user.createdAt
					? new Date(a.user.createdAt).getTime()
					: 0
				const bTime = b.user.createdAt
		...
		const totalPages = total ? Math.max(1, Math.ceil(total / pageSize)) : 1
		const items = scored.slice(offset, offset + pageSize).map(row => row.user)
		...
		switch (params.targetType) {
			...
			case 'DIRECTION':
				condition = targetValue
					? eq(users.direction, String(targetValue))
					: undefined
				break
			case 'VILOYAT':
				condition = targetValue
					? eq(users.viloyat, String(targetValue))
					: undefined
				break
			case 'TUMAN':
				condition = targetValue
					? eq(users.tuman, String(targetValue))
					: undefined
				break
			case 'SHAHAR':
				condition = targetValue
					? eq(users.shahar, String(targetValue))
					: undefined
				break
			case 'MAHALLA':
				condition = targetValue
					? eq(users.mahalla, String(targetValue))
					: undefined
				break
			case 'ALL':
				condition = undefined
	...
	async listTasksWithAssignments(params: {
		...
		const assignmentsByTask = new Map<
			number,
			Array<{ assignment: TaskAssignment; user: User }>
		>()
		...
		const [row] = await db
			...
			.where(
				and(
					...
					sql`${taskAssignments.status} != ${status}`,
				),
			)
			.returning()
		return row ?? null
	...
	async updateAssignmentDelivery(
		id: number,
		deliveredAt: Date,
	): Promise<TaskAssignment> {
		const [row] = await db
